# Databricks notebook source exported at Sat, 22 Oct 2016 22:18:00 UTC
# MAGIC %md
...
def tokenize(string):
    ...
    """
    temp_result = simpleTokenize(string)
    return [item for item in temp_result if item not in stopwords]
...
# TODO: Replace <FILL IN> with appropriate code
amazonRecToToken = amazonSmall.map(lambda (recordID, txt):(recordID, tokenize(txt)))
googleRecToToken = googleSmall.map(lambda (recordID, txt):(recordID, tokenize(txt)))
...
def idfs(corpus):
    ...
    """
    uniqueTokens = corpus.map(lambda (key, value): (key, list(set(value))))
    tokenCountPairTuple = uniqueTokens.flatMap(lambda (key, value): value).map(lambda x: (x, 1))
    tokenSumPairTuple = tokenCountPairTuple.groupByKey().map(lambda (key, value): (key, len(value)))
    N = corpus.count()
    return (tokenSumPairTuple.map(lambda (key, value): (key, 1.0 * N / value)))
...
# TODO
smallIDFTokens = idfsSmall.takeOrdered(11, key=lambda (key, value): value)
...
idfsToCountRow = idfsSmall.map(lambda (key, value): Row(value))
...
def tfidf(tokens, idfs):
    ...
    """
    tfs = tf(tokens)
    tfIdfDict = {key:value*idfs[key] for (key, value) in tfs.items()}
    return tfIdfDict
...
def dotprod(a, b):
    ...
    """
    dotProd = 0
    key_list = [key for key in a.keys() if key in b]
    for key in key_list:
      dotProd += a[key] * b[key]
    return dotProd
...
# TODO: Replace <FILL IN> with appropriate code
sims = similaritiesBroadcast.map(lambda (googleURL, amazonID, cs):("{} {}".format(amazonID, googleURL), cs))

trueDupsRDD = (sims.join(goldStandard).map(lambda (key, value):(key, value[0])))
trueDupsCount = trueDupsRDD.count()

avgSimDups = float(trueDupsRDD.map(lambda (key, value):value).mean())

nonDupsRDD = (sims
              .subtractByKey(goldStandard))
avgSimNon = float(nonDupsRDD.map(lambda (key, value):value).mean())
...
# TODO: Replace <FILL IN> with appropriate code
amazonFullRecToToken = amazon.map(lambda (recordID, sentence):(recordID, tokenize(sentence)))
googleFullRecToToken = google.map(lambda (recordID, sentence):(recordID, tokenize(sentence)))
...
# TODO: Replace <FILL IN> with appropriate code
amazonNorms = amazonWeightsRDD.map(lambda (key, value): (key, norm(value))).collectAsMap()
amazonNormsBroadcast = sc.broadcast(amazonNorms)
googleNorms = googleWeightsRDD.map(lambda (key, value): (key, norm(value))).collectAsMap()
